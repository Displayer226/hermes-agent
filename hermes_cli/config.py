...
DEFAULT_CONFIG = {
    ...
    # and 1.5s defaults were a touch short for HTTP/OAuth servers on a cold
    # connect; a 3s bound lets more of them land in the FIRST turn's snapshot.
    # This is only a turn-1 latency/UX knob: a server that misses this window
    # is still picked up automatically on the next turn by the between-turns refresh
