...
def _schedule_mcp_late_refresh(sid: str, agent) -> None:
    ...
    ``mcp_discovery_timeout`` config value, default 3.0s) so
...
def _make_agent(
    ...
    #
    # Default timeout is 3s. Override with ``mcp_discovery_timeout`` in
    # config.yaml when a remote/slow MCP server (e.g. SillyTavern session
    # proxy) needs more headroom before the agent snapshots its tool list.
    try:
